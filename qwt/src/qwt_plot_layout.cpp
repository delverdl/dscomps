...
void QwtPlotLayout::LayoutData::init( const QwtPlot *plot, const QRectF &rect )
{
    ...
    // canvas
    QMargins mrg = plot->canvas()->contentsMargins();

    canvas.contentsMargins[ QwtPlot::yLeft ] = mrg.left();
    canvas.contentsMargins[ QwtPlot::xTop ] = mrg.top();
    canvas.contentsMargins[ QwtPlot::yRight ] = mrg.right();
    canvas.contentsMargins[ QwtPlot::xBottom ] = mrg.bottom();
}
...
QSize QwtPlotLayout::minimumSizeHint( const QwtPlot *plot ) const
{
    ...
    int fw = plot->canvas()->contentsMargins().left();
    ...
    QMargins mrg = canvas->contentsMargins( );
