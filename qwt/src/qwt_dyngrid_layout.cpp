...
int QwtDynGridLayout::maxRowWidth( int numColumns ) const
{
    ...
    int rowWidth = 2 * contentsMargins().left() + ( numColumns - 1 ) * spacing();
    for ( col = 0; col < numColumns; col++ )
    ...
    uint numColumns ) const
{
    ...
    rowY[0] = yOffset + contentsMargins().top();
    for ( uint r = 1; r < numRows; r++ )
        rowY[r] = rowY[r-1] + rowHeight[r-1] + xySpace;

    colX[0] = xOffset + contentsMargins().left();
...
int QwtDynGridLayout::heightForWidth( int width ) const
{
    ...
    int h = 2 * contentsMargins().top() + ( numRows - 1 ) * spacing();
    ...
    uint numColumns, QVector<int>& rowHeight, QVector<int>& colWidth ) const
{
    ...
    if ( expandH )
    {
        int xDelta = rect.width() - 2 * contentsMargins().left() - ( numColumns - 1 ) * spacing();
    ...
    if ( expandV )
    {
        ...
        int yDelta = rect.height() - 2 * contentsMargins().top() - ( numRows - 1 ) * spacing();
        for ( uint row = 0; row < numRows; row++ )
...
QSize QwtDynGridLayout::sizeHint() const
{
    ...
    int w = 2 * contentsMargins().left() + ( numColumns - 1 ) * spacing();
    for ( uint col = 0; col < numColumns; col++ )
        w += colWidth[col];
