    ...
    const QRectF &rect, bool fillBackground ) const
{
    ...
    QRect layoutRect;
    layoutRect.setLeft( qCeil( rect.left() ) + mrg.left());
    layoutRect.setTop( qCeil( rect.top() ) + mrg.top() );
    layoutRect.setRight( qFloor( rect.right() ) - mrg.right() );
    layoutRect.setBottom( qFloor( rect.bottom() ) - mrg.bottom() );
