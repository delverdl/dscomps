...
bool KZip::closeArchive()
{
    ...
    while (it.hasNext()) {
        ...
        if (d->m_extraField == ModificationTime) {
            ...
            unsigned long time = (unsigned long)it.value()->date().secsSinceEpoc();
                            ...
                            const QDateTime &accessTime, const QDateTime &modificationTime, const QDateTime &creationTime)
{
    ...
    uint atime = (uint) accessTime.secsSinceEpoc();
    uint mtime = (uint) modificationTime.secsSinceEpoc();
    uint ctime = (uint) creationTime.secsSinceEpoc();
