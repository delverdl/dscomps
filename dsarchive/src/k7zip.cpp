...
QByteArray K7Zip::K7ZipPrivate::readAndDecodePackedStreams(bool readMainStreamInfo)
{
    ...
    for (int i = 0; i < folders.size(); i++) {
        ...
        for (int j = 0; j < seqInStreams.size(); ++j) {
            ...
            filter->setInBuffer(deflatedData.data(), static_cast<uint>(deflatedData.size()));

            QByteArray outBuffer;
            // reserve memory
            outBuffer.resize(static_cast<int>(unpackSize));
            ...
            while (result != KFilterBase::End && result != KFilterBase::Error && !filter->inBufferEmpty()) {
                filter->setOutBuffer(outBuffer.data(), static_cast<uint>(outBuffer.size()));
...
void K7Zip::K7ZipPrivate::createItemsFromEntities(const KArchiveDirectory *dir, const QString &path, QByteArray &data)
{
    ...
    for (; it != l.end(); ++it) {
        ...
        mTimes.append(rtlSecondsSince1970ToSpecTime(entry->date().secsSinceEpoc()));
...
{
    header.append(static_cast<char>(b));
    countSize++;
}
...
void K7Zip::K7ZipPrivate::writeFolder(const Folder *folder)
{
    ...
    for (int i = 0; i < folder->folderInfos.size(); i++) {
        ...
        {
            size_t propsSize = static_cast<size_t>(info->properties.size());
            ...
            int longID[15];
            for (int t = static_cast<int>(idSize) - 1; t >= 0; t--, id >>= 8) {
            ...
            for (size_t j = 0; j < propsSize; ++j) {
                writeByte(info->properties[static_cast<int>(j)]);
            }
        }
    }
