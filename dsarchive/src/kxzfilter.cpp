...
void KXzFilter::setOutBuffer(char *data, uint maxlen)
{
    ...
    d->zStream.next_out = reinterpret_cast<uint8_t *>(data);
}

void KXzFilter::setInBuffer(const char *data, unsigned int size)
{
    d->zStream.avail_in = size;
    d->zStream.next_in = reinterpret_cast<uint8_t*>(const_cast<char *>(data));
}

int KXzFilter::inBufferAvailable() const
{
    return static_cast<int>(d->zStream.avail_in);
}

int KXzFilter::outBufferAvailable() const
{
    return static_cast<int>(d->zStream.avail_out);
}
