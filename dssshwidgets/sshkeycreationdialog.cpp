...
void SshKeyCreationDialog::generateKeys()
{
    ...
    if (!m_keyGenerator)
        m_keyGenerator = new SshKeyGenerator(BIND_MEM_CB(
                                               &SshKeyCreationDialog::retrievePassword,
                                               this)
                                             );
...
QString SshKeyCreationDialog::retrievePassword(bool& ok)
{
    QInputDialog d;
    ...
    d.setOkButtonText(tr("Ok"));
    d.setCancelButtonText(tr("Cancel"));
