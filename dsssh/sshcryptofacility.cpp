    ...
    QList<BigInt> &pubKeyParams, QList<BigInt> &allKeyParams, QString &error)
{
    try {
        ...
        m_authKey.reset(PKCS8::load_key(pipe, m_rng, SshKeyPasswordRetriever(m_funcPassword)));
...
SshEncryptionFacility::SshEncryptionFacility(SshPasswordCall funcPassword)
: SshAbstractCryptoFacility(), m_funcPassword(funcPassword)
{ }
