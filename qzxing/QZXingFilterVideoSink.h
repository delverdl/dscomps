...
class QZXingFilter : public QObject
{
    ...
    Q_PROPERTY(QZXing* decoder READ getDecoder NOTIFY decoderChanged)
    Q_PROPERTY(QRectF captureRect MEMBER captureRect NOTIFY captureRectChanged)
    Q_PROPERTY(QObject* videoSink READ getVideoSink WRITE setVideoSink NOTIFY videoSinkChanged)
    ...
    signals:
        ...
        void decoderChanged();   //Unused signal
        void videoSinkChanged(); //Unused signal
