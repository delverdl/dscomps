...
QSharedPointer<DecoderResult> Decoder::decode(QSharedPointer<zxing::aztec::AztecDetectorResult> detectorResult)
{
  ...
  for (size_t i = 0; i < (size_t) aCorrectedBits->getSize(); i++)
  {
  ...
  for (int i = 0; i < numDataCodewords; i++)
  {
    ...
    for (int j = 0; j < codewordSize_; j++)
    {
      ...
      else
      {
        ...
        if (color)
          correctedBits->set(i * codewordSize_ + j - offset);
      }
...
QSharedPointer<BitArray> Decoder::extractBits(QSharedPointer<zxing::BitMatrix> matrix)
{
  ...
  if (ddata_->isCompact())
  {
    if (ddata_->getNBLayers() > 5)
    { //NB_BITS_COMPACT length
