...
int mp_exptmod_fast(const mp_int *G, const mp_int *X, const mp_int *P, mp_int *Y, int redmode)
{
   ...
   if (redmode == 0) {
      ...
      if ((((P->used * 2) + 1) < (int)MP_WARRAY) &&
          (P->used < (1ll << ((CHAR_BIT * sizeof(mp_word)) - (2 * DIGIT_BIT))))) {
   ...
   /* compute the value at M[1ll<<(winsize-1)] by squaring M[1] (winsize-1) times */
   ...
   /* create upper table */
   for (x = (1ll << (winsize - 1)) + 1; x < (1ll << winsize); x++) {
